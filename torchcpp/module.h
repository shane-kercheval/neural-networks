...
public:
    ...
    */
    virtual MatrixXd forward(const MatrixXd& x) = 0;  // = 0 means pure virtual function in C++ which means that the derived class must implement the function.
    ...
    */
    MatrixXd backward(const MatrixXd& grad_output) {
        if (!training) {
    ...
    */
    void step(const std::function<void(MatrixXd&, MatrixXd&)>& optimizer) {
        if (!training) {
...
protected:
    virtual MatrixXd backward_impl(const MatrixXd& grad_output) = 0;
    /**
     * @brief Update the parameters of the module using the gradient computed during
     * backpropagation and the optimizer provided.
     *
    ...
    */
    virtual void step_imp(const std::function<void(MatrixXd&, MatrixXd&)>& optimizer) {}
    /**
