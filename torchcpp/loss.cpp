...
double CrossEntropyLoss::forward(const MatrixXd& logits, const VectorXi& targets) {
    ...
    const long num_examples = logits.rows();
    double loss = 0.0;
    const double clipped = 1e-15;
    for (long i = 0; i < num_examples; ++i) {
        double prob = softmax_probs(i, targets(i));
        loss += -std::log(std::max(prob, clipped));
    }
    return loss / static_cast<double>(num_examples);
}
...
Eigen::MatrixXd CrossEntropyLoss::backward() {
    ...
    const long num_examples = logits_cache_.rows();
    if (num_examples == 0) {
    ...
    for (long i = 0; i < targets_cache_.size(); ++i) {
        grad_logits(i, targets_cache_(i)) -= 1;
    }
    grad_logits /= static_cast<double>(targets_cache_.size());
    return grad_logits;
}
